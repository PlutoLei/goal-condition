...
每个 Attempt 必须先持久化 LaunchIntent 与 target-root lease；LaunchIntent 绑定当前 controller release digest 与 target root 的 canonical path、device、inode，再在同一事务把 intent/session 置为 `dispatching/Dispatching`，之后才能调用既有 Codex launcher；dispatch 前版本和物理身份再核，claim 后任何崩溃或重试只 readback，绝不重发。没有 `write` Authority 的 Attempt 使用 `read-only` sandbox；获授 `write` 才能使用 `workspace-write`。receipt 以 controller 新建 thread 的空 turn 集为前态，并绑定一次 `turn/start` 的响应 ID、带随机 controller correlation 的精确输入 SHA-256 与完成后的持久化 turn 历史。单一持久化 turn 使用 LaunchReceipt v2；若产品在同一次 controller start 后自动续成多 turn，只有根 turn 唯一 user text 的哈希核回、后续 turn 全部零 user-message、`turn/started` 增量与完整有序链长度相等，才使用 LaunchReceipt v3。finalize 前后必须保持同一有序链；任何后续用户输入、malformed input、计数不符、重复、缺失、重排或新增 turn 都 fail closed，不能 Certified。
