...
任务级自动授权由已确认 contract 的可选 `execution_permissions` 编译进独立 `--settings`：postflight 命令编译成**精确** `Bash(<整条 argv 拼接>)`（含权限 DSL 无法表示的字符——括号、换行、首尾空白——则跳过该条，不静默通配、不炸；执行体主动跑该 verifier 会被 deny、可续），显式 `bash_prefixes` 生成前缀 `Bash(<prefix>:*)`，另生成 WebFetch domain、Skill 与额外目录。**不从 verifier 的 `argv[0]` 通配整个可执行家族**——那会让 `git diff` 顺带授权 `git push`、`bash -lc` 授权任意 shell。未知字段、Codex runtime 误用和无法安全放进权限 DSL 的字符串都在 Validate 阶段红。统一的 DSL 表示性校验也覆盖 realpath 后的 target root、state root 与 hook path；编译器拒绝不可表示值，launch 前置闸再独立检查一次，直接绕过 Validate 也不能放行（realpath 后才含不可表示字符的路径由 `prepareClaude` 包成干净 `PERMISSION_SPECIFIER_UNREPRESENTABLE` 诊断，不是裸 TypeError）。deny 固定保护 hook、整个 state 目录，以及每个 realpath target root 下的 `.claude/settings.json` / `.claude/settings.local.json`（F-B1）。Claude 的实际 cwd 使用同一 canonical 首 root；控制器绑定全部 target root **与 `additional_read_roots`** 的 device/inode，并在 spawn 前复核，避免 symlink 重定向或目录替换把执行送进未受对应 deny 保护的仓库；resume 另绑 launch-time cwd（pointer 记录的 canonical cwd 与此刻重算不一致即拒，防 target root 被重定向后在别处复活会话）。
