...
4. **project settings 的发现范围锚在 cwd。** 预存 settings 扫描只扫每个 target root 自身的 `.claude/settings*.json`，不向上扫祖先——依据是实测（2.1.229，deny 探针）：cwd 是 git 仓库子目录时，git root 的 `.claude/settings.json` **不被加载**（同一条 `Read(//abs)` deny 放在 cwd 生效、放在祖先不生效；deny 优先级最高、不可能被别的配置抵消，因此「不生效」只能是「未加载」）。CLI 若未来把 discovery 改成向上找 git root，扫描范围就出现一个静默缺口且无任何直检——**升版后怀疑权限面时这条假设要一并重验**（`additional_read_roots` 目录内预存 settings 是否被加载未实测，现行扫描也不覆盖它，记为已知边界）。
