...
当前 app-server 的 `turn/start` 响应 ID 与 `thread/read` 持久化 ID 可能不同，不能把二者强行视为同一个字段，也不能把任意 readback turn 洗入授权。controller 新建 thread 时先读取空 turn 集，在待发送文本中加入随机 256-bit `Controller Turn Correlation`，保存响应 ID 与 exact text SHA-256。若首次 `thread/read(includeTurns=true)` 在同一 thread 上精确出现一个持久化 turn，且其中唯一 user message 的文本哈希核回 `turn_input_sha256`，则创建严格单元素的 LaunchReceipt v2。若一个 controller `turn/start` 被产品自动续成多个持久化 turn，只有根 turn 满足同一输入核回、后续每个 turn 都为零 user-message 的 `continuation`、`turn/started` 通知数与完整链长度相等，才创建 LaunchReceipt v3；其 `authorized_turn_ids` 是有序、唯一、以 `turn_id` 为首的完整链。GoalSession schema 仍为 v2，receipt v2/v3 可并存且无需迁移。

verify、reconcile、rollout canary 与 finalize 前后两道 turn fence 都逐项比较 receipt 的完整有序链。初始非空、根输入不符、后续含任何 user message、malformed input、通知数不符、重复/缺失/重排/新增 turn 均不得推断为 continuation：出现未授权输入或 turn 时记为 `CONTROL_PLANE_BYPASS`，其余不可归因形态进入 `ReconciliationRequired`。随后 controller 在 default-deny Seatbelt 中执行 active Conditions：只读显式系统 runtime 依赖与 target roots、唯一可写 verifier 临时目录、无网络、最小环境、有界进程/CPU/文件资源，不能检查或 signal 宿主进程；启动使用结构化 argv 与 `shell:false`，Condition 仍可显式声明 `/bin/sh -c`，但不会发生隐式 shell 拼接。Evidence 绑定 root baseline、当前 context、runtime version、projection、snapshot 与 Attempt。详见 [continuation-lineage attribution v3 ADR](../../docs/decisions/2026-08-17-codex-continuation-lineage-attribution-v3.md)。
